...
startroot = Tk()
width= startroot.winfo_screenwidth()
height= startroot.winfo_screenheight()
#setting tkinter window size
startroot.geometry("%dx%d" % (width, height))
startroot.title('Login Page')
startroot.configure(bg="#fff")
...
menubar = Menu(startroot)
startroot.config(menu=menubar)

spimg1 = PhotoImage(file='G:\\Peoples\\Shruti\\BCCA Project\\Code\\images\\picture1.png')
Label(startroot,image=spimg1,bg='white').place(x=0,y=0)



def startp():
	root = Toplevel(startroot)
	...
	def signin():
		...
		if username=='shruti' and password=='1234':
			...
			menubar.add_command(label='Help',command=helping)
			spimg1 = PhotoImage(file='G:\\Peoples\\Shruti\\BCCA Project\\Code\\images\\picture1.png')
...
menubar.add_command(label='Login',command=startp)
